<x-app-quranquiz>
    ...
    @if (session('success'))
        <div class="mb-6 text-sm rounded-md px-4 py-3" style="background-color: #E8F0E9; color: var(--ink-deep); border: 1px solid var(--ink-deep);">
            {{ session('success') }}
        </div>
    @endif
    ...
    <!-- Buat Quiz Baru -->
    <div class="rounded-sm p-6 mb-6" style="background-color: white; border: 1px solid var(--parchment-border); border-top: 3px solid var(--gold-leaf);">
        ...
        <form method="POST" action="{{ route('admin.generate-questions.store-quiz') }}" class="flex flex-col sm:flex-row gap-3">
            ...
            <input type="number" name="seconds_per_question" value="30" min="5" max="300"
                   ...
                   style="border: 1px solid var(--parchment-border); --tw-ring-color: var(--gold-leaf);"
                   title="Detik per soal" required>
            <button type="submit"
                    class="rounded-md px-5 py-2.5 text-sm font-semibold whitespace-nowrap transition-opacity hover:opacity-90"
        ...
        @else
            <form method="POST" action="{{ route('admin.generate-questions.generate') }}" id="generate-form">
                ...
                <button type="submit" id="generate-btn"
                        class="w-full rounded-md px-5 py-3 text-sm font-semibold transition-opacity hover:opacity-90"
    ...
    <!-- Daftar Quiz -->
    <div class="rounded-sm overflow-hidden" style="background-color: white; border: 1px solid var(--parchment-border); border-top: 3px solid var(--gold-leaf);">
                    ...
                    <div class="flex items-center justify-between px-6 py-4" style="border-top: 1px solid var(--parchment-border);">
                        ...
                        <div class="flex gap-2">
                            <a href="{{ route('admin.generate-questions.preview', $quiz) }}"
                               class="rounded-md px-3 py-1.5 text-xs font-semibold transition-colors hover:bg-amber-50"
                            ...
                            <form method="POST" action="{{ route('admin.generate-questions.destroy-quiz', $quiz) }}"
                                ...
                                @method('DELETE')
                                <button type="submit"
                                        class="rounded-md px-3 py-1.5 text-xs font-semibold transition-colors hover:bg-red-50"
