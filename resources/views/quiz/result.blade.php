<x-app-quranquiz>
    ...
    <!-- Kartu skor utama -->
    <div class="manuscript-frame rounded-sm p-10 text-center mb-8 relative overflow-hidden" style="background-color: white;">
        ...
        <p class="font-display text-6xl mb-1 relative" style="color: var(--ink-deep);">{{ $attempt->score }}</p>
    ...
    <div class="flex gap-3 mb-10">
        <a href="{{ route('quiz.leaderboard', $attempt->quiz_id) }}"
           class="flex-1 text-center rounded-md px-4 py-3 text-sm font-semibold transition-opacity hover:opacity-90"
           style="background-color: var(--ink-deep); color: var(--parchment);">
            Lihat Peringkat
        </a>
        <a href="{{ route('quiz.index') }}"
           class="flex-1 text-center rounded-md px-4 py-3 text-sm font-semibold transition-colors hover:bg-amber-50"
           style="border: 1px solid var(--gold-leaf); color: var(--maroon);">
            Kuis Lainnya
        </a>
    </div>

    <h3 class="font-display text-xl mb-4" style="color: var(--ink-deep);">Rincian Jawaban</h3>
    <div class="space-y-3">
